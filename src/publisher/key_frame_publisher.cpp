                                     ...
                                     int buff_size) : nh_(nh), frame_id_(frame_id)
{
    ...
    // 第1个参数表示Publisher节点将会发布以topic_name为话题的String类型消息
    // 第2个参数表示消息发布队列的大小，当发布消息的实际速度较慢时，Publisher会将消息存储在一定空间的队列中；
...
void KeyFramePublisher::Publish(KeyFrame& key_frame)
{
    ...
    // 发布消息的旋转部分
    Eigen::Quaternionf q = key_frame.GetQuaternion();       // 得到关键帧的旋转部分
