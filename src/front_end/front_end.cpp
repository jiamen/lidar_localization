...
namespace lidar_localization
{
    ...
    {
        std::string filter_mothod = config_node[filter_user + "_filter"].as<std::string>();     // voxel_filter
        LOG(INFO) << filter_user << "选择的滤波方法为：" << filter_mothod;

        if (filter_mothod == "voxel_filter")
        {
            // 选择体素滤波器
            filter_ptr = std::make_shared<VoxelFilter>(config_node[filter_mothod][filter_user]);    // voxel_filter:  local_map:
        }
        else
        {
    ...
    bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose)
    {
        ...
        // 不是第一帧，就正常匹配          输入待匹配点云     初始位姿T_rc          目标地图点云       当前输入帧点云到目标点云的优化变换位姿T_rc
        registration_ptr_->ScanMatch(filtered_cloud_ptr, predict_pose, result_cloud_ptr_, current_frame_.pose);
        cloud_pose = current_frame_.pose;               // ENU坐标系下的 T_rc
